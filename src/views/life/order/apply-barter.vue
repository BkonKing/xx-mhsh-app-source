  ...
  <div class="app-body">
    ...
    <div class="order-session">
      <div class="cont-session goods-session">
        <div class="can-apply">
          <div v-for="(item, index) in ableList" class="apply-goods" :key="index">
            <div
              :class="[
                item.is_checked ? 'cur' : '',
                'cart-checkbox flex-center'
              ]"
              @click="checkboxOne(index)"
              data-index=""
            >
            ...
            <div class="order-goods-info">
              ...
              <div class="order-info">
                ...
                <div class="order-action-session">
                  ...
                  <div class="order-buy-num">×{{item.specs_count}}</div>
                </div>
              </div>
              <div
                v-if="item.is_checked"
                @click.stop="showFunc(index + 1)"
                class="apply-select flex-between"
              >
        ...
        <div v-if="unableList.length > 0" class="not-apply">
          <div
            @click="toggle"
            :class="[toggleDown ? 'cur' : '', 'apply-toggle']"
          >
            部分商品不支持换货({{ unableList.length }})
          </div>
          <div
            v-show="!toggleDown"
          ...
          <div class="shipping-address-item">
            ...
            <div class="shipping-address-item-right">
              <div class="shipping-address-text p-nowrap">
                {{ addressInfo.address_name + addressInfo.address_house }}
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    getData () {
      getApplyBarter({
        ...
        if (res.success) {
          ...
          this.skuList = res.data.data_ok[0]
            ? res.data.data_ok[0].goods_specs_list
            : []
    ...
    skuSure () {
      ...
      this.ableList[this.tabIndex].sku_name = this.skuList[
        this.typeVal
      ].specs_name
      this.showFunc()
      this.total()
