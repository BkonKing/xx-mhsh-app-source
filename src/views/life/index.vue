<template>
  <div class="app-body">
    ...
    <div
      :class="[
        activeIndex > 0 && navList2.length ? 'seconds-nav-show' : '',
        'fixed-empty'
      ]"
    ></div>
    ...
    <div
      :class="[
        activeIndex > 0 && navList2.length ? 'scroll-body-two' : '',
        'scroll-body'
      ]"
      id="life-body"
    >
          ...
          <template v-if="activeIndex == 0">
            ...
            <template v-for="(item, index) in lifeData">
              <template v-if="item.type == 2">
                ...
                >
                  <div
                    class="life-tit life-special-tit flex-between"
                    @click="linkFunc(3)"
                  >
                    <div
                      class="font-34 color-fff font-weight flex-align-center"
                    >
                      <span>{{ item.bargain_name }}</span>
                    </div>
                    <div class="life-arrow-right">
                      <img class="img-100" src="@/assets/img/right_02.png" />
                    </div>
                  </div>
                  <div class="special-goods-list">
                    ...
                    >
                      ...
                      <div class="life-goods-price">
                        ￥{{ val.te_price / 100 }}
                        <span>￥{{ val.s_price / 100 }}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <template v-else-if="item.type == 1">
                ...
                >
                  <div
                    class="life-tit life-flash-tit flex-between"
                    @click="linkFunc(2)"
                  >
                    <div
                      class="font-34 color-fff font-weight flex-align-center"
                  ...
                  <div class="flash-goods-list">
                    <div
                      v-for="(val, key) in item.child"
                      @click="linkFunc(5, { id: val.goods_id })"
                      class="life-goods-item flex-between"
                    >
                      <div class="life-goods-pic">
                        <img class="img-100" :src="val.thumb" />
                      </div>
                      ...
                      <div class="item-btn">
                        <template v-if="item.ollage_info.is_start == 1">
                          <template v-if="val.is_over == 0">
                            <div
                              class="btn-collage"
                              v-if="val.is_collage && val.order_project_id != 0"
                            >
                              邀请拼单
                            </div>
                        ...
                        <template v-else>
                          <div
                            ...
                            @click.stop="remindFunc(index, key, val.goods_id)"
              ...
              <template v-else>
                <template v-if="item.special_type == 2">
                  <div
                    v-if="item.child && item.child.length > 0"
                    class="special-session flex-between"
                  >
                    <div class="special-list">
                      <template
                        v-if="item.child.length < 4"
                        v-for="(val, key) in item.child"
                      >
                        <div
                          @click="linkFunc(4, { id: val.special_id })"
                          class="height-345"
                        >
                    ...
                    <div class="life-goods-list flex-align-center">
                      ...
                      >
                        ...
                        <div class="life-goods-price">
                          ...
                          <span
                            v-if="
                              val.original_price && val.original_price != '0'
                            "
                            >￥{{ val.original_price / 100 }}</span
                          >
                        </div>
                      </div>
                    </div>
  ...
  methods: {
        ...
        if (res.success) {
          ...
          if (this.isRefresh) {
            ...
            if (navArr.length && res.data.category_list.length) {
              if (
                JSON.stringify(navArr) != JSON.stringify(res.data.category_list)
              ) {
                this.navList = [];
                this.navList2 = [];
                this.category_id = "";
          ...
          if (this.navList.length == 0) {
            ...
            this.isEmpty =
              this.page == 1 && res.data.goods_list.length == 0 ? true : false;
    ...
    sureSwal: function(e) {
      ...
      remindSend({
        ...
        if (res.success) {
          Toast(res.message);
          that.lifeData[that.set_item_index].child[
            that.set_index
          ].is_set = true;
        }
      });
